...
exports.uploadUserData_cb = function(req, res) {
	...
	var startGeneratingMovie = function() {
		...
		if ( req.session.user.userId ) {
			...
			aeServerManager.createMovie_longPolling(aeServer, req.body.project_ID, req.body.userId._id, req.body.userId.fb_userID, videoTitle);
	...
	var writeTo_customized_content_xml_cb = function (err) {
		if (!err) {
			...
			if ( allUserContentExist ) {
				...
				aeServerManager.createMovie_longPolling(aeServer, req.body.projectID, req.body.ownerID._id, req.body.ownerID.fb_userID, videoTitle);
			}
			else {
